...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-full transition-colors">
      {/* Header */}
      <header className="px-6 pt-12 pb-4 sticky top-0 z-10 bg-transparent">
      ...
      <div className="px-6 pb-48 space-y-8 flex-1 overflow-y-auto">
        ...
        {/* Balance Card */}
        <div className="bg-slate-950 dark:bg-slate-900 border border-slate-800 rounded-[2rem] p-7 text-white shadow-2xl shadow-indigo-900/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-500/30 rounded-full blur-3xl transform translate-x-1/3 -translate-y-1/3"></div>
          <div className="absolute bottom-0 left-0 w-32 h-32 bg-purple-500/20 rounded-full blur-2xl transform -translate-x-1/2 translate-y-1/2"></div>
          
          <div className="relative z-10">
            <p className="text-slate-300 dark:text-slate-400 text-sm font-medium mb-2">Gastado este mes</p>
            <h2 className="text-4xl font-extrabold tracking-tight mb-8">{formatMoney(spent)}</h2>
            
            <div className="space-y-3">
              <div className="h-2.5 w-full bg-slate-800 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-1000 ease-out ${percent >= 100 ? 'bg-gradient-to-r from-red-500 to-rose-400' : percent >= 80 ? 'bg-gradient-to-r from-amber-500 to-orange-400' : 'bg-gradient-to-r from-indigo-500 to-purple-500'}`}
                  style={{ width: `${clampedPercent}%` }}
                ></div>
              </div>
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-400">Presupuesto: {formatMoney(budget)}</span>
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            Gestionar Categorías y Presupuesto
          </button>
        </div>
      ...
      <div className="fixed bottom-[88px] left-0 w-full flex justify-center px-4 pointer-events-none z-40">
        <div className="w-full max-w-md pointer-events-auto">
          <form onSubmit={handleExpenseSubmit} className="bg-slate-900/95 dark:bg-slate-800/95 p-3 rounded-[24px] shadow-2xl flex flex-col gap-2 border border-slate-800/50 backdrop-blur-xl transition-colors">
            
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 px-1">
              <button
                type="button"
                onClick={() => setSelectedCategory('')}
                className={`whitespace-nowrap px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors flex-shrink-0 ${!selectedCategory ? 'bg-indigo-500 text-white shadow-md' : 'bg-slate-800 dark:bg-slate-700 text-slate-300 hover:bg-slate-700'}`}
              >
                ✨ Auto
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`whitespace-nowrap px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors flex flex-shrink-0 items-center gap-1.5 ${selectedCategory === cat.name ? 'bg-indigo-500 text-white shadow-md' : 'bg-slate-800 dark:bg-slate-700 text-slate-300 hover:bg-slate-700'}`}
                >
            ...
            <div className="flex items-center gap-2">
              <input 
                ...
                className="flex-1 bg-slate-800/50 dark:bg-slate-900/50 border border-slate-700 rounded-xl text-white px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-400 text-sm font-medium transition-all"
